...
s32 Collider_InitBase(GameState* state, Collider* collider) {
...
s32 Collider_DestroyBase(GameState* state, Collider* collider) {
    return 1;
}
 ...
 */
s32 Collider_SetBaseToActor(GameState* state, Collider* collider, ColliderInitToActor* src) {
 ...
 */
s32 Collider_SetBaseType1(GameState* state, Collider* collider, Actor* actor, ColliderInitType1* src) {
...
s32 Collider_SetBase(GameState* state, Collider* collider, Actor* actor, ColliderInit* src) {
    collider->actor = actor;
    collider->colType = src->colType;
    collider->atFlags = src->atFlags;
    collider->acFlags = src->acFlags;
...
void Collider_ResetATBase(GameState* state, Collider* collider) {
    collider->at = NULL;
    collider->atFlags &= ~(AT_HIT | AT_BOUNCED);
}

void Collider_ResetACBase(GameState* state, Collider* collider) {
    collider->ac = NULL;
    collider->acFlags &= ~(AC_HIT | AC_BOUNCED);
}

void Collider_ResetOCBase(GameState* state, Collider* collider) {
    collider->oc = NULL;
    collider->ocFlags1 &= ~OC1_HIT;
    collider->ocFlags2 &= ~OC2_HIT_PLAYER;
}

s32 Collider_InitTouch(GameState* state, ColliderTouch* touch) {
...
s32 Collider_DestroyTouch(GameState* state, ColliderTouch* touch) {
    return 1;
}

s32 Collider_SetTouch(GameState* state, ColliderTouch* dest, ColliderTouch* src) {
...
void Collider_ResetATInfo_Unk(GameState* state, ColliderInfo* info) {
}

s32 Collider_InitBump(GameState* state, ColliderBump* bump) {
...
s32 Collider_DestroyBump(GameState* state, ColliderBump* bump) {
    return 1;
}

s32 Collider_SetBump(GameState* state, ColliderBump* bump, ColliderBumpInit* init) {
...
s32 Collider_InitInfo(GameState* state, ColliderInfo* info) {
    ...
    *info = init;
    Collider_InitTouch(state, &info->toucher);
    Collider_InitBump(state, &info->bumper);
    return 1;
}

s32 Collider_DestroyInfo(GameState* state, ColliderInfo* info) {
    Collider_DestroyTouch(state, &info->toucher);
    Collider_DestroyBump(state, &info->bumper);
    return 1;
}

s32 Collider_SetInfo(GameState* state, ColliderInfo* info, ColliderInfoInit* infoInit) {
    info->elemType = infoInit->elemType;
    Collider_SetTouch(state, &info->toucher, &infoInit->toucher);
    Collider_SetBump(state, &info->bumper, &infoInit->bumper);
...
void Collider_ResetATInfo(GameState* state, ColliderInfo* info) {
    info->atHit = NULL;
    info->atHitInfo = NULL;
    info->toucherFlags &= ~TOUCH_HIT;
    info->toucherFlags &= ~TOUCH_DREW_HITMARK;
    Collider_ResetATInfo_Unk(state, info);
}

void Collider_ResetACInfo(GameState* state, ColliderInfo* info) {
...
void Collider_ResetOCInfo(GameState* state, ColliderInfo* info) {
    info->ocElemFlags &= ~OCELEM_HIT;
}

s32 Collider_InitJntSphElementDim(GameState* state, ColliderJntSphElementDim* dim) {
...
s32 Collider_DestroyJntSphElementDim(GameState* state, ColliderJntSphElementDim* element) {
    return 1;
}

s32 Collider_SetJntSphElementDim(GameState* state, ColliderJntSphElementDim* dest,
...
s32 Collider_InitJntSphElement(GameState* state, ColliderJntSphElement* element) {
    Collider_InitInfo(state, &element->info);
    Collider_InitJntSphElementDim(state, &element->dim);
    return 1;
}

s32 Collider_DestroyJntSphElement(GameState* state, ColliderJntSphElement* element) {
    Collider_DestroyInfo(state, &element->info);
    Collider_DestroyJntSphElementDim(state, &element->dim);
    return 1;
}

s32 Collider_SetJntSphElement(GameState* state, ColliderJntSphElement* dest, ColliderJntSphElementInit* src) {
    Collider_SetInfo(state, &dest->info, &src->info);
    Collider_SetJntSphElementDim(state, &dest->dim, &src->dim);
    return 1;
}

s32 Collider_ResetJntSphElementAT(GameState* state, ColliderJntSphElement* collider) {
    Collider_ResetATInfo(state, &collider->info);
    return 1;
}

s32 Collider_ResetJntSphElementAC(GameState* state, ColliderJntSphElement* collider) {
    Collider_ResetACInfo(state, &collider->info);
    return 1;
}

s32 Collider_ResetJntSphElementOC(GameState* state, ColliderJntSphElement* collider) {
    Collider_ResetOCInfo(state, &collider->info);
    return 1;
}
 ...
 */
s32 Collider_InitJntSph(GameState* state, ColliderJntSph* collider) {
    Collider_InitBase(state, &collider->base);
    collider->count = 0;
 ...
 */
s32 Collider_FreeJntSph(GameState* state, ColliderJntSph* collider) {
    ColliderJntSphElement* element;

    Collider_DestroyBase(state, &collider->base);
    for (element = collider->elements; element < collider->elements + collider->count; element++) {
        Collider_DestroyJntSphElement(state, element);
    }
 ...
 */
s32 Collider_DestroyJntSph(GameState* state, ColliderJntSph* collider) {
    ColliderJntSphElement* element;
 ...
 */
s32 Collider_SetJntSphToActor(GameState* state, ColliderJntSph* dest, ColliderJntSphInitToActor* src) {
    ColliderJntSphElement* destElem;
    ColliderJntSphElementInit* srcElem;

    Collider_SetBaseToActor(state, &dest->base, &src->base);
    ...
    for (destElem = dest->elements, srcElem = src->elements; destElem < dest->elements + dest->count;
         destElem++, srcElem++) {
        Collider_InitJntSphElement(state, destElem);
        Collider_SetJntSphElement(state, destElem, srcElem);
    }
    return 1;
}
 ...
 */
s32 Collider_SetJntSphAllocType1(GameState* state, ColliderJntSph* dest, Actor* actor,
                                 ColliderJntSphInitType1* src) {
    ColliderJntSphElement* destElem;
    ColliderJntSphElementInit* srcElem;

    Collider_SetBaseType1(state, &dest->base, actor, &src->base);
    dest->count = src->count;
 ...
 * Unused.
 */
s32 Collider_SetJntSphAlloc(GameState* state, ColliderJntSph* dest, Actor* actor, ColliderJntSphInit* src) {
    ColliderJntSphElement* destElem;
    ColliderJntSphElementInit* srcElem;

    Collider_SetBase(state, &dest->base, actor, &src->base);
    dest->count = src->count;
 ...
 */
s32 Collider_SetJntSph(GameState* state, ColliderJntSph* dest, Actor* actor, ColliderJntSphInit* src,
 ...
 */
s32 Collider_ResetJntSphAT(GameState* state, Collider* collider) {
    ColliderJntSphElement* element;
    ColliderJntSph* jntSph = (ColliderJntSph*)collider;

    Collider_ResetATBase(state, &jntSph->base);

    for (element = jntSph->elements; element < jntSph->elements + jntSph->count; element++) {
        Collider_ResetJntSphElementAT(state, element);
    }
    return 1;
}
 ...
 */
s32 Collider_ResetJntSphAC(GameState* state, Collider* collider) {
    ColliderJntSphElement* element;
    ColliderJntSph* jntSph = (ColliderJntSph*)collider;

    Collider_ResetACBase(state, &jntSph->base);

    for (element = jntSph->elements; element < jntSph->elements + jntSph->count; element++) {
        Collider_ResetJntSphElementAC(state, element);
    }
    return 1;
}
 ...
 */
s32 Collider_ResetJntSphOC(GameState* state, Collider* collider) {
    ColliderJntSphElement* element;
    ColliderJntSph* jntSph = (ColliderJntSph*)collider;

    Collider_ResetOCBase(state, &jntSph->base);

    for (element = jntSph->elements; element < jntSph->elements + jntSph->count; element++) {
        Collider_ResetJntSphElementOC(state, element);
    }
    return 1;
}

s32 Collider_InitCylinderDim(GameState* state, Cylinder16* dim) {
...
s32 Collider_DestroyCylinderDim(GameState* state, Cylinder16* dim) {
    return 1;
}

s32 Collider_SetCylinderDim(GameState* state, Cylinder16* dest, Cylinder16* src) {
    *dest = *src;
    return 1;
}
 ...
 */
s32 Collider_InitCylinder(GameState* state, ColliderCylinder* collider) {
    Collider_InitBase(state, &collider->base);
    Collider_InitInfo(state, &collider->info);
    Collider_InitCylinderDim(state, &collider->dim);
    return 1;
}
 ...
 */
s32 Collider_DestroyCylinder(GameState* state, ColliderCylinder* collider) {
    Collider_DestroyBase(state, &collider->base);
    Collider_DestroyInfo(state, &collider->info);
    Collider_DestroyCylinderDim(state, &collider->dim);
    return 1;
}
 ...
 */
s32 Collider_SetCylinderToActor(GameState* state, ColliderCylinder* collider,
                                ColliderCylinderInitToActor* src) {
    Collider_SetBaseToActor(state, &collider->base, &src->base);
    Collider_SetInfo(state, &collider->info, &src->info);
    Collider_SetCylinderDim(state, &collider->dim, &src->dim);
    return 1;
}
 ...
 */
s32 Collider_SetCylinderType1(GameState* state, ColliderCylinder* collider, Actor* actor,
                              ColliderCylinderInitType1* src) {
    Collider_SetBaseType1(state, &collider->base, actor, &src->base);
    Collider_SetInfo(state, &collider->info, &src->info);
    Collider_SetCylinderDim(state, &collider->dim, &src->dim);
    return 1;
}
 ...
 */
s32 Collider_SetCylinder(GameState* state, ColliderCylinder* collider, Actor* actor,
                         ColliderCylinderInit* src) {
    Collider_SetBase(state, &collider->base, actor, &src->base);
    Collider_SetInfo(state, &collider->info, &src->info);
    Collider_SetCylinderDim(state, &collider->dim, &src->dim);
    return 1;
}
 ...
 * Resets the collider's AT collision flags.
 */
s32 Collider_ResetCylinderAT(GameState* state, Collider* collider) {
    ColliderCylinder* cylinder = (ColliderCylinder*)collider;

    Collider_ResetATBase(state, &cylinder->base);
    Collider_ResetATInfo(state, &cylinder->info);
    return 1;
}
 ...
 * Resets the collider's AC collision flags.
 */
s32 Collider_ResetCylinderAC(GameState* state, Collider* collider) {
    ColliderCylinder* cylinder = (ColliderCylinder*)collider;

    Collider_ResetACBase(state, &cylinder->base);
    Collider_ResetACInfo(state, &cylinder->info);
    return 1;
}
 ...
 * Resets the collider's OC collision flags.
 */
s32 Collider_ResetCylinderOC(GameState* state, Collider* collider) {
    ColliderCylinder* cylinder = (ColliderCylinder*)collider;

    Collider_ResetOCBase(state, &cylinder->base);
    Collider_ResetOCInfo(state, &cylinder->info);
    return 1;
}

s32 Collider_InitTrisElementDim(GameState* state, TriNorm* dim) {
    static TriNorm init = {
...
s32 Collider_DestroyTrisElementDim(GameState* state, TriNorm* dim) {
    return 1;
}

s32 Collider_SetTrisElementDim(GameState* state, TriNorm* dest, ColliderTrisElementDimInit* src) {
    Vec3f* destVtx;
    Vec3f* srcVtx;
    f32 nx;
    f32 ny;
...
s32 Collider_InitTrisElement(GameState* state, ColliderTrisElement* collider) {
    Collider_InitInfo(state, &collider->info);
    Collider_InitTrisElementDim(state, &collider->dim);
    return 1;
}

s32 Collider_DestroyTrisElement(GameState* state, ColliderTrisElement* collider) {
    Collider_DestroyInfo(state, &collider->info);
    Collider_DestroyTrisElementDim(state, &collider->dim);
    return 1;
}

s32 Collider_SetTrisElement(GameState* state, ColliderTrisElement* dest, ColliderTrisElementInit* src) {
    Collider_SetInfo(state, &dest->info, &src->info);
    Collider_SetTrisElementDim(state, &dest->dim, &src->dim);
    return 1;
}

s32 Collider_ResetTrisElementAT(GameState* state, ColliderTrisElement* element) {
    Collider_ResetATInfo(state, &element->info);
    return 1;
}

s32 Collider_ResetTrisElementAC(GameState* state, ColliderTrisElement* element) {
    Collider_ResetACInfo(state, &element->info);
    return 1;
}

s32 Collider_ResetTrisElementOC(GameState* state, ColliderTrisElement* element) {
    Collider_ResetOCInfo(state, &element->info);
    return 1;
}
 ...
 */
s32 Collider_InitTris(GameState* state, ColliderTris* tris) {
    Collider_InitBase(state, &tris->base);
    tris->count = 0;
    tris->elements = 0;
    return 1;
}
 ...
 * Unused
 */
s32 Collider_FreeTris(GameState* state, ColliderTris* tris) {
    ColliderTrisElement* element;

    Collider_DestroyBase(state, &tris->base);
    for (element = tris->elements; element < tris->elements + tris->count; element++) {
        Collider_DestroyTrisElement(state, element);
    }
 ...
 */
s32 Collider_DestroyTris(GameState* state, ColliderTris* tris) {
    ColliderTrisElement* element;
 ...
 * Unused.
 */
s32 Collider_SetTrisAllocType1(GameState* state, ColliderTris* dest, Actor* actor, ColliderTrisInitType1* src) {
    ...
    for (destElem = dest->elements, srcElem = src->elements; destElem < dest->elements + dest->count;
         destElem++, srcElem++) {
        Collider_InitTrisElement(state, destElem);
        Collider_SetTrisElement(state, destElem, srcElem);
    }
    return 1;
}
 ...
 * Unused
 */
s32 Collider_SetTrisAlloc(GameState* state, ColliderTris* dest, Actor* actor, ColliderTrisInit* src) {
    ColliderTrisElement* destElem;
    ColliderTrisElementInit* srcElem;
 ...
 */
s32 Collider_SetTris(GameState* state, ColliderTris* dest, Actor* actor, ColliderTrisInit* src,
 ...
 * Resets the collider's AT collision flags.
 */
s32 Collider_ResetTrisAT(GameState* state, Collider* collider) {
    ColliderTrisElement* element;
    ColliderTris* tris = (ColliderTris*)collider;

    Collider_ResetATBase(state, &tris->base);
    for (element = tris->elements; element < tris->elements + tris->count; element++) {
        Collider_ResetTrisElementAT(state, element);
    }
    return 1;
}
 ...
 * Resets the collider's AC collision flags.
 */
s32 Collider_ResetTrisAC(GameState* state, Collider* collider) {
    ColliderTrisElement* element;
    ColliderTris* tris = (ColliderTris*)collider;

    Collider_ResetACBase(state, &tris->base);
    for (element = tris->elements; element < tris->elements + tris->count; element++) {
        Collider_ResetTrisElementAC(state, element);
    }
    return 1;
}
 ...
 * Resets the collider's OC collision flags.
 */
s32 Collider_ResetTrisOC(GameState* state, Collider* collider) {
    ColliderTrisElement* element;
    ColliderTris* tris = (ColliderTris*)collider;

    Collider_ResetOCBase(state, &tris->base);
    for (element = tris->elements; element < tris->elements + tris->count; element++) {
        Collider_ResetTrisElementOC(state, element);
    }
    return 1;
}

s32 Collider_InitQuadDim(GameState* state, ColliderQuadDim* dim) {
...
s32 Collider_DestroyQuadDim(GameState* state, ColliderQuadDim* dim) {
    return 1;
}

s32 Collider_ResetQuadACDist(GameState* state, ColliderQuadDim* dim) {
    dim->acDist = 1.0E38f;
    return 1;
}
...
s32 Collider_SetQuadDim(GameState* state, ColliderQuadDim* dest, ColliderQuadDimInit* src) {
 ...
 */
s32 Collider_InitQuad(GameState* state, ColliderQuad* collider) {
    Collider_InitBase(state, &collider->base);
    Collider_InitInfo(state, &collider->info);
    Collider_InitQuadDim(state, &collider->dim);
    return 1;
}
 ...
 */
s32 Collider_DestroyQuad(GameState* state, ColliderQuad* collider) {
    Collider_DestroyBase(state, &collider->base);
    Collider_DestroyInfo(state, &collider->info);
    Collider_DestroyQuadDim(state, &collider->dim);
    return 1;
}
 ...
 */
s32 Collider_SetQuadType1(GameState* state, ColliderQuad* collider, Actor* actor, ColliderQuadInitType1* src) {
    Collider_SetBaseType1(state, &collider->base, actor, &src->base);
    Collider_SetInfo(state, &collider->info, &src->info);
    Collider_SetQuadDim(state, &collider->dim, &src->dim);
    return 1;
}
 ...
 */
s32 Collider_SetQuad(GameState* state, ColliderQuad* collider, Actor* actor, ColliderQuadInit* src) {
    Collider_SetBase(state, &collider->base, actor, &src->base);
    Collider_SetInfo(state, &collider->info, &src->info);
    Collider_SetQuadDim(state, &collider->dim, &src->dim);
    return 1;
 ...
 * Resets the collider's AT collision flags.
 */
s32 Collider_ResetQuadAT(GameState* state, Collider* collider) {
    ColliderQuad* quad = (ColliderQuad*)collider;

    Collider_ResetATBase(state, &quad->base);
    Collider_ResetATInfo(state, &quad->info);
    Collider_ResetQuadACDist(state, &quad->dim);
    return 1;
}
 ...
 * Resets the collider's AC collision flags.
 */
s32 Collider_ResetQuadAC(GameState* state, Collider* collider) {
    ColliderQuad* quad = (ColliderQuad*)collider;

    Collider_ResetACBase(state, &quad->base);
    Collider_ResetACInfo(state, &quad->info);
    return 1;
}
 ...
 * Resets the collider's OC collision flags.
 */
s32 Collider_ResetQuadOC(GameState* state, Collider* collider) {
    ColliderQuad* quad = (ColliderQuad*)collider;

    Collider_ResetOCBase(state, &quad->base);
    Collider_ResetOCInfo(state, &quad->info);
    return 1;
}
 ...
 */
s32 Collider_QuadSetNearestAC(GameState* state, ColliderQuad* quad, Vec3f* hitPos) {
    f32 acDist;
    Vec3f dcMid;
    ...
    if (acDist < quad->dim.acDist) {
        ...
        if (quad->info.atHit != NULL) {
            Collider_ResetACBase(state, quad->info.atHit);
        }
        if (quad->info.atHitInfo != NULL) {
            Collider_ResetACInfo(state, quad->info.atHitInfo);
        }
        return true;
    }
    return false;
 ...
 */
s32 Collider_InitLine(GameState* state, OcLine* line) {
 ...
 * OcLines are entirely unused.
 */
s32 Collider_DestroyLine(GameState* state, OcLine* line) {
    return 1;
}
 ...
 * OcLines are entirely unused.
 */
s32 Collider_SetLinePoints(GameState* state, OcLine* ocLine, Vec3f* a, Vec3f* b) {
 ...
 * OcLines are entirely unused.
 */
s32 Collider_SetLine(GameState* state, OcLine* dest, OcLine* src) {
    dest->ocFlags = src->ocFlags;
    Collider_SetLinePoints(state, dest, &src->line.a, &src->line.b);
    return 1;
}
 ...
 * OcLines are entirely unused.
 */
s32 Collider_ResetLineOC(GameState* state, OcLine* line) {
 ...
 */
void CollisionCheck_InitContext(GameState* state, CollisionCheckContext* colChkCtx) {
    colChkCtx->sacFlags = 0;
    CollisionCheck_ClearContext(state, colChkCtx);
    AREG(21) = true;
    AREG(22) = true;
    AREG(23) = true;
}

void CollisionCheck_DestroyContext(GameState* state, CollisionCheckContext* colChkCtx) {
}

/**
 * Clears all collider lists in CollisionCheckContext when not in SAC mode.
 */
void CollisionCheck_ClearContext(GameState* state, CollisionCheckContext* colChkCtx) {
    Collider** col;
    OcLine** line;
 ...
 */
void CollisionCheck_EnableSAC(GameState* state, CollisionCheckContext* colChkCtx) {
 ...
 */
void CollisionCheck_DisableSAC(GameState* state, CollisionCheckContext* colChkCtx) {
    colChkCtx->sacFlags &= ~AC_ON;
}
 ...
 */
void Collider_Draw(GameState* state, Collider* collider) {
    ...
    switch (collider->shape) {
        case COLSHAPE_JNTSPH:
            ...
            for (i = 0; i < jntSph->count; i++) {
                Math3D_DrawSphere(state, &jntSph->elements[i].dim.worldSphere);
            }
            break;
        case COLSHAPE_CYLINDER:
            cylinder = (ColliderCylinder*)collider;
            Math3D_DrawCylinder(state, &cylinder->dim);
            break;
        case COLSHAPE_TRIS:
            tris = (ColliderTris*)collider;
            for (i = 0; i < tris->count; i++) {
                Collider_DrawRedPoly(state->gfxCtx, &tris->elements[i].dim.vtx[0],
                                     &tris->elements[i].dim.vtx[1], &tris->elements[i].dim.vtx[2]);
            }
            break;
        case COLSHAPE_QUAD:
            quad = (ColliderQuad*)collider;
            Collider_DrawRedPoly(state->gfxCtx, &quad->dim.quad[2], &quad->dim.quad[3], &quad->dim.quad[1]);
            Collider_DrawRedPoly(state->gfxCtx, &quad->dim.quad[1], &quad->dim.quad[0], &quad->dim.quad[2]);
            break;
    }
}
 ...
 */
void CollisionCheck_DrawCollision(GameState* state, CollisionCheckContext* colChkCtx) {
    ...
    if (AREG(15)) {
        if (AREG(21)) {
            for (i = 0; i < colChkCtx->colATCount; i++) {
                Collider_Draw(state, colChkCtx->colAT[i]);
            }
        }
        if (AREG(22)) {
            for (i = 0; i < colChkCtx->colACCount; i++) {
                Collider_Draw(state, colChkCtx->colAC[i]);
            }
        }
        if (AREG(23)) {
            for (i = 0; i < colChkCtx->colOCCount; i++) {
                ...
                if (collider->ocFlags1 & OC1_ON) {
                    Collider_Draw(state, collider);
                }
            }
        }
        if (AREG(24)) {
            BgCheck_DrawDynaCollision(state, &globalCtx->colCtx);
        }
        if (AREG(25)) {
            BgCheck_DrawStaticCollision(state, &globalCtx->colCtx);
        }
    }
}
...
s32 CollisionCheck_SetOC(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ...
    sOCResetFuncs[collider->shape](state, collider);
    if (collider->actor != NULL && collider->actor->update == NULL) {
        return -1;
    }
 ...
 */
void CollisionCheck_NoBlood(GameState* state, Collider* collider, Vec3f* v) {
}
 ...
 */
void CollisionCheck_RedBlood(GameState* state, Collider* collider, Vec3f* v) {
    CollisionCheck_SpawnRedBlood(state, v);
}
 ...
 * Unused.
 */
void CollisionCheck_RedBloodUnused(GameState* state, Collider* collider, Vec3f* v) {
    CollisionCheck_SpawnRedBlood(state, v);
}
...
void CollisionCheck_HitEffects(GameState* state, Collider* at, ColliderInfo* atInfo, Collider* ac,
    ...
    if (ac->actor != NULL) {
        sBloodFuncs[sHitInfo[ac->colType].blood](state, ac, hitPos);
    }
    if (ac->actor != NULL) {
        if (sHitInfo[ac->colType].effect == HIT_SOLID) {
            CollisionCheck_HitSolid(state, atInfo, ac, hitPos);
        } else if (sHitInfo[ac->colType].effect == HIT_WOOD) {
            if (at->actor == NULL) {
                CollisionCheck_SpawnShieldParticles(state, hitPos);
                Audio_PlaySoundGeneral(NA_SE_IT_REFLECTION_WOOD, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
            } else {
 ...
 */
s32 CollisionCheck_SetATvsAC(GameState* state, Collider* at, ColliderInfo* atInfo, Vec3f* atPos, Collider* ac,
    ...
    if (!(atInfo->toucherFlags & TOUCH_AT_HITMARK) && ac->colType != COLTYPE_METAL && ac->colType != COLTYPE_WOOD &&
        ...
        CollisionCheck_HitEffects(state, at, atInfo, ac, acInfo, hitPos);
 ...
 */
void CollisionCheck_AC_JntSphVsJntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                      Collider* colAC) {
        ...
        for (atItem = at->elements; atItem < at->elements + at->count; atItem++) {
            ...
            for (acElem = ac->elements; acElem < ac->elements + ac->count; acElem++) {
                ...
                if (Math3D_SphVsSphOverlapCenter(&atItem->dim.worldSphere, &acElem->dim.worldSphere, &overlapSize,
                    ...
                    CollisionCheck_SetATvsAC(state, &at->base, &atItem->info, &atPos, &ac->base, &acElem->info,
                                             &acPos, &hitPos);
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_JntSphVsCyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                   Collider* colAC) {
    ColliderJntSph* at = (ColliderJntSph*)colAT;
    ColliderJntSphElement* atItem;
    ...
    if (at->count > 0 && at->elements != NULL && ac->dim.radius > 0 && ac->dim.height > 0) {
        ...
        for (atItem = at->elements; atItem < at->elements + at->count; atItem++) {
            ...
            if (Math3D_SphVsCylOverlapCenterDist(&atItem->dim.worldSphere, &ac->dim, &overlapSize, &centerDist)) {
                ...
                CollisionCheck_SetATvsAC(state, &at->base, &atItem->info, &atPos, &ac->base, &ac->info, &acPos,
                                         &hitPos);
                return;
            }
        }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_CylVsJntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                   Collider* colAC) {
    ...
    if (ac->count > 0 && ac->elements != NULL && at->dim.radius > 0 && at->dim.height > 0) {
        ...
        for (acElem = ac->elements; acElem < ac->elements + ac->count; acElem++) {
            ...
            if (Math3D_SphVsCylOverlapCenterDist(&acElem->dim.worldSphere, &at->dim, &overlapSize, &centerDist)) {
                ...
                CollisionCheck_SetATvsAC(state, &at->base, &at->info, &atPos, &ac->base, &acElem->info, &acPos,
                                         &hitPos);
                if (!(ac->base.ocFlags2 & OC2_FIRST_ONLY)) {
                    break;
                }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_JntSphVsTris(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                    Collider* colAC) {
    ColliderJntSph* at = (ColliderJntSph*)colAT;
        ...
        for (atSph = at->elements; atSph < at->elements + at->count; atSph++) {
            ...
            for (acTri = ac->elements; acTri < ac->elements + ac->count; acTri++) {
                ...
                if (Math3D_TriVsSphIntersect(&atSph->dim.worldSphere, &acTri->dim, &hitPos) == 1) {
                    ...
                    CollisionCheck_SetATvsAC(state, &at->base, &atSph->info, &atPos, &ac->base, &acTri->info,
                                             &acPos, &hitPos);
                    return;
                }
            }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_TrisVsJntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                    Collider* colAC) {
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_JntSphVsQuad(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                    Collider* colAC) {
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_QuadVsJntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                    Collider* colAC) {
    ...
    if (ac->count > 0 && ac->elements != NULL) {
        ...
        for (acElem = ac->elements; acElem < ac->elements + ac->count; acElem++) {
            ...
            if (Math3D_TriVsSphIntersect(&acElem->dim.worldSphere, &D_8015E2A0, &hitPos) == 1 ||
                ...
                if (Collider_QuadSetNearestAC(state, at, &hitPos)) {
                    Vec3f atPos;
                    Vec3f acPos;
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_CylVsCyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                Collider* colAC) {
    ColliderCylinder* at = (ColliderCylinder*)colAT;
    ColliderCylinder* ac = (ColliderCylinder*)colAC;
    f32 deadSpace;
    ...
    if (at->dim.radius > 0 && at->dim.height > 0 && ac->dim.radius > 0 && ac->dim.height > 0) {
        ...
        if (Math3D_CylOutsideCylDist(&at->dim, &ac->dim, &deadSpace, &centerDistXZ) == 1) {
            ...
            CollisionCheck_SetATvsAC(state, &at->base, &at->info, &atPos, &ac->base, &ac->info, &acPos, &hitPos);
        }
    }
}
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_CylVsTris(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                 Collider* colAC) {
    ColliderCylinder* at = (ColliderCylinder*)colAT;
    ColliderTris* ac = (ColliderTris*)colAC;
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_TrisVsCyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                 Collider* colAC) {
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_CylVsQuad(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                 Collider* colAC) {
    ...
    if (at->dim.height > 0 && at->dim.radius > 0) {
        ...
        if (Math3D_CylTriVsIntersect(&at->dim, &D_8015E320, &D_8015E390) == 1) {
            ...
            CollisionCheck_SetATvsAC(state, &at->base, &at->info, &atPos1, &ac->base, &ac->info, &acPos1,
                                     &D_8015E390);
            ...
            CollisionCheck_SetATvsAC(state, &at->base, &at->info, &atPos2, &ac->base, &ac->info, &acPos2,
                                     &D_8015E390);
        }
    }
}
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_QuadVsCyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                 Collider* colAC) {
    ...
    if (ac->dim.height > 0 && ac->dim.radius > 0) {
        ...
        if (Math3D_CylTriVsIntersect(&ac->dim, &D_8015E3A0, &D_8015E410) == 1) {
            if (Collider_QuadSetNearestAC(state, at, &D_8015E410)) {
                Vec3f atPos1;
                Vec3f acPos1;
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_TrisVsTris(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                  Collider* colAC) {
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_TrisVsQuad(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                  Collider* colAC) {
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_QuadVsTris(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                  Collider* colAC) {
    ...
    if (ac->count > 0 && ac->elements != NULL) {
        ...
        for (acElem = ac->elements; acElem < ac->elements + ac->count; acElem++) {
            ...
            if (Math3D_TriVsTriIntersect(&D_8015E4C0, &acElem->dim, &D_8015E4B0) == 1 ||
                ...
                if (Collider_QuadSetNearestAC(state, at, &D_8015E4B0)) {
                    Vec3f atPos;
                    Vec3f acPos;
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void CollisionCheck_AC_QuadVsQuad(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT,
                                  Collider* colAC) {
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 2; j++) {
            if (Math3D_TriVsTriIntersect(&D_8015E5A8[j], &D_8015E530[i], &D_8015E598) == 1) {
                if (Collider_QuadSetNearestAC(state, at, &D_8015E598)) {
                    Vec3f atPos;
                    Vec3f acPos;
                    ...
                    CollisionCheck_SetATvsAC(state, &at->base, &at->info, &atPos, &ac->base, &ac->info, &acPos,
                                             &D_8015E598);
                    return;
                }
            }
 ...
 */
void CollisionCheck_SetJntSphHitFX(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderJntSph* jntSph = (ColliderJntSph*)collider;
    ColliderJntSphElement* element;
    Vec3f hitPos;
    ...
    for (element = jntSph->elements; element < jntSph->elements + jntSph->count; element++) {
        if ((element->info.bumperFlags & BUMP_DRAW_HITMARK) && (element->info.acHitInfo != NULL) &&
            ...
            CollisionCheck_HitEffects(state, element->info.acHit, element->info.acHitInfo, &jntSph->base,
                                      &element->info, &hitPos);
            element->info.acHitInfo->toucherFlags |= TOUCH_DREW_HITMARK;
            return;
        }
 ...
 */
void CollisionCheck_SetCylHitFX(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderCylinder* cylinder = (ColliderCylinder*)collider;
    Vec3f hitPos;
    ...
    if ((cylinder->info.bumperFlags & BUMP_DRAW_HITMARK) && (cylinder->info.acHitInfo != NULL) &&
        ...
        CollisionCheck_HitEffects(state, cylinder->info.acHit, cylinder->info.acHitInfo, &cylinder->base,
 ...
 */
void CollisionCheck_SetTrisHitFX(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderTris* tris = (ColliderTris*)collider;
    ColliderTrisElement* element;
    Vec3f hitPos;
    ...
    for (element = tris->elements; element < tris->elements + tris->count; element++) {
        if ((element->info.bumperFlags & BUMP_DRAW_HITMARK) && (element->info.acHitInfo != NULL) &&
            !(element->info.acHitInfo->toucherFlags & TOUCH_DREW_HITMARK)) {
            Math_Vec3s_ToVec3f(&hitPos, &element->info.bumper.hitPos);
            CollisionCheck_HitEffects(state, element->info.acHit, element->info.acHitInfo, &tris->base,
                                      &element->info, &hitPos);
            element->info.acHitInfo->toucherFlags |= TOUCH_DREW_HITMARK;
            return;
        }
 ...
 */
void CollisionCheck_SetQuadHitFX(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderQuad* quad = (ColliderQuad*)collider;
    Vec3f hitPos;
    ...
    if ((quad->info.bumperFlags & BUMP_DRAW_HITMARK) && (quad->info.acHitInfo != NULL) &&
        ...
        CollisionCheck_HitEffects(state, quad->info.acHit, quad->info.acHitInfo, &quad->base, &quad->info, &hitPos);
        quad->info.acHitInfo->toucherFlags |= TOUCH_DREW_HITMARK;
    }
}
 ...
 */
void CollisionCheck_SetHitEffects(GameState* state, CollisionCheckContext* colChkCtx) {
    Collider** col;
    ...
    for (col = colChkCtx->colAC; col < colChkCtx->colAC + colChkCtx->colACCount; col++) {
        ...
        if (colAC != NULL && colAC->acFlags & AC_ON) {
            ...
            sColChkApplyFuncs[colAC->shape](state, colChkCtx, colAC);
        }
    }
}
 ...
 */
void CollisionCheck_AC(GameState* state, CollisionCheckContext* colChkCtx, Collider* colAT) {
    Collider** col;
    ...
    for (col = colChkCtx->colAC; col < colChkCtx->colAC + colChkCtx->colACCount; col++) {
        ...
        if (colAC != NULL && colAC->acFlags & AC_ON) {
            ...
            if ((colAC->acFlags & colAT->atFlags & AC_TYPE_ALL) && (colAT != colAC)) {
                ...
                sACVsFuncs[colAT->shape][colAC->shape](state, colChkCtx, colAT, colAC);
            }
        }
    }
}
 ...
 */
void CollisionCheck_AT(GameState* state, CollisionCheckContext* colChkCtx) {
    Collider** col;
    ...
    for (col = colChkCtx->colAT; col < colChkCtx->colAT + colChkCtx->colATCount; col++) {
        ...
        if (colAT != NULL && colAT->atFlags & AT_ON) {
            ...
            CollisionCheck_AC(state, colChkCtx, colAT);
        }
    }
    CollisionCheck_SetHitEffects(state, colChkCtx);
}
 ...
 */
void CollisionCheck_OC_JntSphVsJntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* l,
                                      Collider* r) {
 ...
 */
void CollisionCheck_OC_JntSphVsCyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* l,
                                   Collider* r) {
    ColliderJntSph* left = (ColliderJntSph*)l;
    ColliderJntSphElement* leftElem;
 ...
 */
void CollisionCheck_OC_CylVsJntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* l,
                                   Collider* r) {
    CollisionCheck_OC_JntSphVsCyl(state, colChkCtx, r, l);
}

/**
 * OC overlap check for two Cylinders
 */
void CollisionCheck_OC_CylVsCyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* l, Collider* r) {
 ...
 */
void CollisionCheck_OC(GameState* state, CollisionCheckContext* colChkCtx) {
    Collider** left;
    Collider** right;
    ColChkVsFunc vsFunc;
    ...
    for (left = colChkCtx->colOC; left < colChkCtx->colOC + colChkCtx->colOCCount; left++) {
        ...
        for (right = left + 1; right < colChkCtx->colOC + colChkCtx->colOCCount; right++) {
            ...
            vsFunc(state, colChkCtx, *left, *right);
        }
    }
}
 ...
 */
void CollisionCheck_ApplyDamage(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider,
                                ColliderInfo* info) {
    DamageTable* tbl;
    f32 damage;
 ...
 */
void CollisionCheck_ApplyDamageJntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderJntSph* jntSph = (ColliderJntSph*)collider;
    s32 i;

    if (jntSph->count > 0 && jntSph->elements != NULL) {
        for (i = 0; i < jntSph->count; i++) {
            CollisionCheck_ApplyDamage(state, colChkCtx, &jntSph->base, &jntSph->elements[i].info);
        }
    }
}
 ...
 */
void CollisionCheck_ApplyDamageCyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderCylinder* cylinder = (ColliderCylinder*)collider;
    CollisionCheck_ApplyDamage(state, colChkCtx, &cylinder->base, &cylinder->info);
}

/**
 * Apply ColliderTris AC damage effect
 */
void CollisionCheck_ApplyDamageTris(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderTris* tris = (ColliderTris*)collider;
    s32 i;

    for (i = 0; i < tris->count; i++) {
        CollisionCheck_ApplyDamage(state, colChkCtx, collider, &tris->elements[i].info);
    }
}
 ...
 */
void CollisionCheck_ApplyDamageQuad(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider) {
    ColliderQuad* quad = (ColliderQuad*)collider;
    CollisionCheck_ApplyDamage(state, colChkCtx, &quad->base, &quad->info);
}
 ...
 * CollisionCheckInfo.
 */
void CollisionCheck_Damage(GameState* state, CollisionCheckContext* colChkCtx) {
    s32 i;
    ...
    for (i = 0; i < colChkCtx->colACCount; i++) {
        ...
        sApplyDamageFuncs[collider->shape](state, colChkCtx, collider);
    }
}
 ...
 */
s32 CollisionCheck_LineOC_JntSph(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider,
                                 Vec3f* a, Vec3f* b) {
 ...
 */
s32 CollisionCheck_LineOC_Cyl(GameState* state, CollisionCheckContext* colChkCtx, Collider* collider, Vec3f* a,
                              Vec3f* b) {
 ...
 */
s32 CollisionCheck_LineOC(GameState* state, CollisionCheckContext* colChkCtx, Vec3f* a, Vec3f* b,
    ...
    for (col = colChkCtx->colOC; col < colChkCtx->colOC + colChkCtx->colOCCount; col++) {
        ...
        if (lineCheck == NULL) {
            ...
            result = lineCheck(state, colChkCtx, (*col), a, b);
            if (result) {
                break;
            }
        }
 ...
 * otherwise. Unused.
 */
s32 CollisionCheck_LineOCCheckAll(GameState* state, CollisionCheckContext* colChkCtx, Vec3f* a, Vec3f* b) {
    CollisionCheck_LineOC(state, colChkCtx, a, b, NULL, 0);
}
 ...
 */
s32 CollisionCheck_LineOCCheck(GameState* state, CollisionCheckContext* colChkCtx, Vec3f* a, Vec3f* b,
                               Actor** exclusions, s32 numExclusions) {
    CollisionCheck_LineOC(state, colChkCtx, a, b, exclusions, numExclusions);
}
 ...
 */
void Collider_SetTrisDim(GameState* state, ColliderTris* collider, s32 index, ColliderTrisElementDimInit* src) {
    ColliderTrisElement* element = &collider->elements[index];

    Collider_SetTrisElementDim(state, &element->dim, src);
}
 ...
 */
void CollisionCheck_SpawnShieldParticles(GameState* state, Vec3f* v) {
 ...
 */
void CollisionCheck_SpawnShieldParticlesMetal(GameState* state, Vec3f* v) {
    CollisionCheck_SpawnShieldParticles(state, v);
 ...
 */
void CollisionCheck_SpawnShieldParticlesMetalSound(GameState* state, Vec3f* v, Vec3f* pos) {
    CollisionCheck_SpawnShieldParticles(state, v);
 ...
 * Spawns streaks of light and makes a metallic sound
 */
void CollisionCheck_SpawnShieldParticlesMetal2(GameState* state, Vec3f* v) {
    CollisionCheck_SpawnShieldParticlesMetal(state, v);
}

/**
 * Spawns streaks of light and makes a wooden sound
 */
void CollisionCheck_SpawnShieldParticlesWood(GameState* state, Vec3f* v, Vec3f* actorPos) {
